...
namespace BFS
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<(string, string, string)> wigthList = SearchInWigth(sizeM, matrixA, start);
            ShowList(wigthList, "Вершина", "BFS", "Очередь");
        }
        ...
        static List<(string, string, string)> SearchInWigth((int, int) sizeM, int[,] matrix, int start)
        {
            List<(string, string, string)> wigthList = new List<(string, string, string)>();
                        ...
                        for (int j = 0; j < sizeM.Item1; j++)
                        {
                            if (matrix[i, j] == 1 && SearchInList(visited, (j + 1)))
                            {
                                if (count == 1)
                                {
                                    ...
                                    wigthList.Add((Convert.ToString(count), Convert.ToString(start), turnStr));
                                }
                                start = j + 1;
                                turn.Add(start); 
                                visited.Add(start);
